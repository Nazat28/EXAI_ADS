...
def get_and_preprocess_compas(params):
    ...
    compas_df = pd.read_csv('modified_data.csv')
    ...
    X = compas_df[['pos_x', 'pos_y', 'pos_z', 'spd_x', 'spd_y', 'spd_z']]

    # if the person has a high score, give them the _negative_ model outcome
    y = np.array([NEGATIVE_OUTCOME if attackerType == 1 else POSITIVE_OUTCOME for attackerType in compas_df['attackerType']])
    sens = X['pos_x']
...
categorical_features = [i for i,f in enumerate(features) if f not in ['pos_y', 'pos_z', 'spd_x']]
###
